...
public class NativeLibraryHelper {
    ...
    public static int findSupportedAbi(Handle handle, String[] supportedAbis) {
        ...
        for (long apkHandle : handle.apkHandles) {
            ...
            if (res != PackageManager.NO_NATIVE_LIBRARIES) {
                if (res == -113) {
                    if (finalRes < 0) {
                        finalRes = -113;
                    }
        ...
        if (!path.isDirectory()) {
            ...
            if (path.mkdir()) {
                try {
                    ...
                    StringBuilder stringBuilder2 = new StringBuilder();
                    stringBuilder2.append("Cannot chmod native library directory ");
    ...
    public static int copyNativeBinariesWithOverride(Handle handle, File libraryRoot, String abiOverride) {
        ...
        try {
            ...
            if (handle.multiArch) {
                ...
                StringBuilder stringBuilder;
                if (abiOverride != null) {
                    if (!str2.equals(abiOverride)) {
                        Slog.w(str, "Ignoring abiOverride for multi arch application.");
                    }
                }
                if (Build.SUPPORTED_32_BIT_ABIS.length > 0) {
                    copyRet = copyNativeBinariesForSupportedAbi(handle, libraryRoot, Build.SUPPORTED_32_BIT_ABIS, true);
                    if (!(copyRet >= 0 || copyRet == PackageManager.NO_NATIVE_LIBRARIES || copyRet == -113)) {
                        stringBuilder = new StringBuilder();
            ...
            if (copyRet2 < 0 && copyRet2 != PackageManager.NO_NATIVE_LIBRARIES) {
                StringBuilder stringBuilder2 = new StringBuilder();
                stringBuilder2.append("Failure copying native libraries [errorCode=");
                stringBuilder2.append(copyRet2);
            ...
            Slog.e(str, "Copying native libraries failed", e);
            return -110;
        }
    }
    ...
    public static long sumNativeBinariesWithOverride(Handle handle, String abiOverride) throws IOException {
        ...
        String str = "-";
        if (handle.multiArch) {
            if (!(abiOverride == null || str.equals(abiOverride))) {
