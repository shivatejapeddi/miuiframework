...
public class PackageHelper {
    ...
    public static String resolveInstallVolume(Context context, SessionParams params, TestableInterface testInterface) throws IOException {
        ...
        String str = " upgrade";
        String str2 = "Not enough space on existing volume ";
        ...
        if (existingInfo == null || !existingInfo.isSystemApp()) {
            ...
            if (forceAllowOnExternal || sessionParams.installLocation != 1) {
                if (existingInfo != null) {
                    ...
                    stringBuilder2.append("No special requests, but no room on allowed volumes.  allow3rdPartyOnInternal? ");
                    stringBuilder2.append(allow3rdPartyOnInternal);
                    throw new IOException(stringBuilder2.toString());
                }
            } else if (existingInfo != null && !Objects.equals(existingInfo.volumeUuid, StorageManager.UUID_PRIVATE_INTERNAL)) {
                stringBuilder2 = new StringBuilder();
                stringBuilder2.append("Cannot automatically move ");
                stringBuilder2.append(sessionParams.appPackageName);
                stringBuilder2.append(" from ");
                stringBuilder2.append(existingInfo.volumeUuid);
                stringBuilder2.append(" to internal storage");
                throw new IOException(stringBuilder2.toString());
    ...
    public static boolean fitsOnExternal(Context context, SessionParams params) {
        ...
        if (params.sizeBytes > 0 && !primary.isEmulated()) {
            if (Environment.MEDIA_MOUNTED.equals(primary.getState()) && params.sizeBytes <= storage.getStorageBytesUntilLow(primary.getPathFile())) {
                return true;
            }
        }
        return false;
