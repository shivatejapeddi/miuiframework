    ...
    public void findItemsWithShortcutForKey(List<MenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (keyEvent.getKeyData(possibleChars) || i == 67) {
            ...
            for (int i2 = 0; i2 < N; i2++) {
                ...
                if (((modifierState & Menu.SUPPORTED_MODIFIERS_MASK) == (Menu.SUPPORTED_MODIFIERS_MASK & (qwerty ? item.getAlphabeticModifiers() : item.getNumericModifiers()))) && shortcutChar != 0 && ((shortcutChar == possibleChars.meta[0] || shortcutChar == possibleChars.meta[2] || (qwerty && shortcutChar == 8 && i == 67)) && item.isEnabled())) {
                    list.add(item);
                }
            }
        }
