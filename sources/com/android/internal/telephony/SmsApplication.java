...
public final class SmsApplication {
    ...
    private static final class SmsPackageMonitor extends PackageMonitor {
        ...
        Runnable mMsgCallback = new Runnable() {
            public void run() {
                SmsPackageMonitor.this.handlePackageChanged();
            }
        };
        ...
        private void onPackageChanged() {
            BackgroundThread.getHandler().removeCallbacks(this.mMsgCallback);
            BackgroundThread.getHandler().post(this.mMsgCallback);
        }
    ...
    private static Collection<SmsApplicationData> getApplicationCollectionInternal(Context context, int userId) {
        ...
        List<ResolveInfo> smsReceivers = packageManager.queryBroadcastReceiversAsUser(new Intent(Intents.SMS_DELIVER_ACTION), 786432, i);
        HashMap<String, SmsApplicationData> receivers = new HashMap();
        for (ResolveInfo resolveInfo : smsReceivers) {
            ActivityInfo activityInfo3 = resolveInfo.activityInfo;
            if (activityInfo3 != null) {
                if (permission.BROADCAST_SMS.equals(activityInfo3.permission)) {
                    packageName = activityInfo3.packageName;
        ...
        for (ResolveInfo resolveInfo2 : packageManager.queryBroadcastReceiversAsUser(intent, 786432, i)) {
        ...
        packageName = "";
        String str = SCHEME_SMSTO;
        for (ResolveInfo resolveInfo3 : packageManager.queryIntentServicesAsUser(new Intent(TelephonyManager.ACTION_RESPOND_VIA_MESSAGE, Uri.fromParts(str, packageName, null)), 786432, i)) {
        ...
        for (ResolveInfo resolveInfo22 : packageManager.queryIntentActivitiesAsUser(new Intent(Intent.ACTION_SENDTO, Uri.fromParts(str, packageName, null)), 786432, i)) {
            activityInfo = resolveInfo22.activityInfo;
            if (activityInfo != null) {
                smsApplicationData = (SmsApplicationData) receivers.get(activityInfo.packageName);
                if (smsApplicationData != null) {
        ...
        for (ResolveInfo resolveInfo4 : packageManager.queryBroadcastReceiversAsUser(new Intent(Intents.ACTION_DEFAULT_SMS_PACKAGE_CHANGED), 786432, i)) {
            ...
            if (activityInfo4 != null) {
                ...
                if (smsApplicationData5 != null) {
                    smsApplicationData5.mSmsAppChangedReceiverClass = activityInfo4.name;
                }
            }
        }
        for (ResolveInfo resolveInfo5 : packageManager.queryBroadcastReceiversAsUser(new Intent(Intents.ACTION_EXTERNAL_PROVIDER_CHANGE), 786432, i)) {
        ...
        for (ResolveInfo resolveInfo32 : packageManager.queryBroadcastReceiversAsUser(new Intent(Intents.SIM_FULL_ACTION), 786432, i)) {
            ActivityInfo activityInfo5 = resolveInfo32.activityInfo;
            if (activityInfo5 != null) {
                smsApplicationData2 = (SmsApplicationData) receivers.get(activityInfo5.packageName);
                if (smsApplicationData2 != null) {
                    smsApplicationData2.mSimFullReceiverClass = activityInfo5.name;
                }
                i = userId;
            }
        }
        Iterator it = smsReceivers.iterator();
        while (it.hasNext()) {
            activityInfo2 = ((ResolveInfo) it.next()).activityInfo;
            if (activityInfo2 != null) {
                Iterator it2;
                String packageName2 = activityInfo2.packageName;
                smsApplicationData3 = (SmsApplicationData) receivers.get(packageName2);
                if (smsApplicationData3 == null) {
                    it2 = it;
                } else if (smsApplicationData3.isComplete()) {
                    it2 = it;
                } else {
                    StringBuilder stringBuilder = new StringBuilder();
                    it2 = it;
                    ...
                    stringBuilder.append(" lacks required manifest declarations to be a default sms app: ");
                    stringBuilder.append(smsApplicationData3);
    ...
    private static boolean tryFixExclusiveSmsAppops(Context context, SmsApplicationData applicationData, boolean updateIfNeeded) {
        ...
        for (int appOp : DEFAULT_APP_EXCLUSIVE_APPOPS) {
            if (appOps.checkOp(appOp, applicationData.mUid, applicationData.mPackageName) != 0) {
                ...
                stringBuilder.append(AppOpsManager.modeToName(appOp));
    ...
    private static void setDefaultApplicationInternal(String packageName, Context context, int userId) {
        ...
        if (str == null || oldPackageName == null || !str.equals(oldPackageName)) {
            ...
            if (applicationData != null) {
                ...
                ((RoleManager) context2.getSystemService(RoleManager.class)).addRoleHolderAsUser(RoleManager.ROLE_SMS, applicationData.mPackageName, 0, UserHandle.of(userId), AsyncTask.THREAD_POOL_EXECUTOR, new -$$Lambda$SmsApplication$gDx3W-UsTeTFaBSPU-Y_LFPZ9dE(future));
                try {
                    future.get(5, TimeUnit.SECONDS);
                    defaultSmsAppChanged(context);
                } catch (InterruptedException | ExecutionException | TimeoutException e2) {
                    StringBuilder stringBuilder2 = new StringBuilder();
                    stringBuilder2.append("Exception while adding sms role holder ");
                    stringBuilder2.append(applicationData);
    ...
    private static void broadcastSmsAppChange(Context context, UserHandle userHandle, SmsApplicationData oldAppData, SmsApplicationData applicationData) {
        ...
        context.sendBroadcastAsUser(new Intent(Intents.ACTION_DEFAULT_SMS_PACKAGE_CHANGED_INTERNAL), userHandle, permission.MONITOR_DEFAULT_SMS_PACKAGE);
        if (applicationData != null) {
    ...
    private static void replacePreferredActivity(PackageManager packageManager, ComponentName componentName, int userId, String scheme) {
        Uri fromParts = Uri.fromParts(scheme, "", null);
        String str = Intent.ACTION_SENDTO;
        List<ResolveInfo> resolveInfoList = packageManager.queryIntentActivitiesAsUser(new Intent(str, fromParts), 65600, userId);
    ...
    public static boolean shouldWriteMessageForPackage(String packageName, Context context) {
        ...
        if (SmsApplicationInjector.isIgnoreSmsStorageApplication(packageName)) {
            return false;
        }
