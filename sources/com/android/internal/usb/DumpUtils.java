...
public class DumpUtils {
    ...
    public static void writeDevice(DualDumpOutputStream dump, String idName, long id, UsbDevice device) {
        ...
        dump.write(Function.CLASS, 1120986464260L, device.getDeviceClass());
    ...
    private static void writeConfiguration(DualDumpOutputStream dump, String idName, long id, UsbConfiguration configuration) {
        ...
        dump.write(CommonPacketExtension.ATTRIBUTE_NAME, 1155346202627L, configuration.getAttributes());
    ...
    private static void writeInterface(DualDumpOutputStream dump, String idName, long id, UsbInterface iface) {
        ...
        dump.write(Function.CLASS, 1120986464260L, iface.getInterfaceClass());
    ...
    private static void writeEndpoint(DualDumpOutputStream dump, String idName, long id, UsbEndpoint endpoint) {
        ...
        dump.write(CommonPacketExtension.ATTRIBUTE_NAME, 1155346202629L, endpoint.getAttributes());
    ...
    public static void writePort(DualDumpOutputStream dump, String idName, long id, UsbPort port) {
        ...
        String str = "supported_modes";
        if (!dump.isProto()) {
            dump.write(str, 2259152797698L, UsbPort.modeToString(mode));
        } else if (mode == 0) {
            dump.write(str, 2259152797698L, 0);
        } else {
            if ((mode & 3) == 3) {
                dump.write(str, 2259152797698L, 3);
            } else if ((mode & 2) == 2) {
                dump.write(str, 2259152797698L, 2);
            } else if ((mode & 1) == 1) {
                dump.write(str, 2259152797698L, 1);
            }
            if ((mode & 4) == 4) {
                dump.write(str, 2259152797698L, 4);
            }
            if ((mode & 8) == 8) {
                dump.write(str, 2259152797698L, 8);
            }
        }
        dump.end(token);
    }
    ...
    public static void writePortStatus(DualDumpOutputStream dump, String idName, long id, UsbPortStatus status) {
        ...
        String str = "current_mode";
        if (dump.isProto()) {
            dump.write(str, 1159641169922L, status.getCurrentMode());
        } else {
            dump.write(str, 1159641169922L, UsbPort.modeToString(status.getCurrentMode()));
        }
        String str2 = "power_role";
        writePowerRole(dump, str2, 1159641169923L, status.getCurrentPowerRole());
        String str3 = "data_role";
        writeDataRole(dump, str3, 1159641169924L, status.getCurrentDataRole());
        ...
        while (undumpedCombinations != 0) {
            ...
            writePowerRole(dump, str2, 1159641169921L, powerRole);
            writeDataRole(dump, str3, 1159641169922L, dataRole);
