...
import android.provider.Telephony.RcsColumns.RcsFileTransferColumns;
import android.service.print.PrinterCapabilitiesProto;
import com.android.internal.util.dump.DualDumpOutputStream;
import com.miui.whetstone.provider.WhetstoneProviderContract.ServiceManager;
...
public class DumpUtils {
    public static void writePrinterId(DualDumpOutputStream proto, String idName, long id, PrinterId printerId) {
        ...
        com.android.internal.util.dump.DumpUtils.writeComponentName(proto, ServiceManager.SERVICE_NAME, 1146756268033L, printerId.getServiceName());
    ...
    public static void writePrinterCapabilities(Context context, DualDumpOutputStream proto, String idName, long id, PrinterCapabilitiesInfo cap) {
        ...
        for (i = 0; i < numMediaSizes; i++) {
            Context context2 = context;
            DualDumpOutputStream dualDumpOutputStream = proto;
            writeMediaSize(context2, dualDumpOutputStream, "media_sizes", 2246267895810L, (MediaSize) cap.getMediaSizes().get(i));
        }
        i = cap.getResolutions().size();
        for (int i2 = 0; i2 < i; i2++) {
            writeResolution(proto, "resolutions", 2246267895811L, (Resolution) cap.getResolutions().get(i2));
        }
        String str = "color_modes";
        if ((cap.getColorModes() & 1) != 0) {
            proto.write(str, 2259152797700L, 1);
        }
        if ((cap.getColorModes() & 2) != 0) {
            proto.write(str, 2259152797700L, 2);
        }
        str = "duplex_modes";
        if ((cap.getDuplexModes() & 1) != 0) {
            proto.write(str, (long) PrinterCapabilitiesProto.DUPLEX_MODES, 1);
        }
        if ((cap.getDuplexModes() & 2) != 0) {
            proto.write(str, (long) PrinterCapabilitiesProto.DUPLEX_MODES, 2);
        }
        if ((cap.getDuplexModes() & 4) != 0) {
            proto.write(str, (long) PrinterCapabilitiesProto.DUPLEX_MODES, 4);
        }
        proto.end(token);
    }
    ...
    public static void writePrintAttributes(Context context, DualDumpOutputStream proto, String idName, long id, PrintAttributes attributes) {
        ...
        if (res != null) {
            writeResolution(proto, VideoColumns.RESOLUTION, 1146756268035L, res);
        }
    ...
    public static void writePrintDocumentInfo(DualDumpOutputStream proto, String idName, long id, PrintDocumentInfo info) {
        ...
        proto.write(RcsFileTransferColumns.CONTENT_TYPE_COLUMN, 1120986464259L, info.getContentType());
        DualDumpOutputStream dualDumpOutputStream = proto;
        dualDumpOutputStream.write("data_size", 1112396529668L, info.getDataSize());
        proto.end(token);
    }

    public static void writePageRange(DualDumpOutputStream proto, String idName, long id, PageRange range) {
        long token = proto.start(idName, id);
        proto.write(BaseMmsColumns.START, 1120986464257L, range.getStart());
    ...
    public static void writePrintJobInfo(Context context, DualDumpOutputStream proto, String idName, long id, PrintJobInfo printJobInfo) {
        ...
        boolean z = true;
        if (state < 1 || state > 7) {
            dualDumpOutputStream.write(str, 1159641169923L, 0);
        } else {
            dualDumpOutputStream.write(str, 1159641169923L, state);
        }
        ...
        if (attributes != null) {
            writePrintAttributes(context, proto, CommonPacketExtension.ATTRIBUTE_NAME, 1146756268039L, attributes);
        }
