...
public class KernelCpuThreadReaderSettingsObserver extends ContentObserver {
    ...
    public static class UidPredicate implements Predicate<Integer> {
        ...
        public static UidPredicate fromString(String predicateString) throws NumberFormatException {
            ...
            while (i < length) {
                ...
                if (uidRangeMatcher.matches()) {
                    ...
                    stringBuilder.append(uidSpecifier);
        ...
        public boolean test(Integer uid) {
            for (int i = 0; i < this.mAcceptedUidRanges.size(); i++) {
                if (((Range) this.mAcceptedUidRanges.get(i)).contains((Comparable) uid)) {
                    return true;
                }
            }
            return false;
    ...
    public static KernelCpuThreadReaderDiff getSettingsModifiedReader(Context context) {
        ...
        context.getContentResolver().registerContentObserver(Global.getUriFor(Global.KERNEL_CPU_THREAD_READER), false, settingsObserver, 0);
    ...
    private void updateReader() {
        String uidPredicate = TAG;
        if (this.mKernelCpuThreadReader != null) {
            KeyValueListParser parser = new KeyValueListParser(',');
            try {
                parser.setString(Global.getString(this.mContext.getContentResolver(), Global.KERNEL_CPU_THREAD_READER));
                try {
                    ...
                    Slog.w(uidPredicate, "Failed to get UID predicate", e);
                }
            } catch (IllegalArgumentException e2) {
                Slog.e(uidPredicate, "Bad settings", e2);
            }
        }
    }
}
