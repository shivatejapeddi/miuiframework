...
public class MobileRadioPowerCalculator extends PowerCalculator {
    ...
    public MobileRadioPowerCalculator(PowerProfile profile, BatteryStats stats) {
        ...
        String str = PowerProfile.POWER_RADIO_ON;
        if (profile.getAveragePowerOrDefault(str, -1.0d) == -1.0d) {
            ...
            while (true) {
                ...
                dArr3[i3] = profile.getAveragePower(str, i3);
                i3++;
            }
        }
    ...
    public void calculateRemaining(BatterySipper app, BatteryStats stats, long rawRealtimeUs, long rawUptimeUs, int statsType) {
        ...
        power += p;
        j = (this.mStats.getMobileRadioActiveTime(j, i) / 1000) - this.mTotalAppMobileActiveMs;
        if (j > 0) {
        ...
        if (power != 0.0d) {
            if (signalTimeMs != 0) {
                batterySipper.noCoveragePercent = (((double) noCoverageTimeMs) * 100.0d) / ((double) signalTimeMs);
            }
