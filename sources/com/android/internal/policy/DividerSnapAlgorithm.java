...
public class DividerSnapAlgorithm {
    ...
    public DividerSnapAlgorithm(Resources res, int displayWidth, int displayHeight, int dividerSize, boolean isHorizontalDivision, Rect insets, int dockSide, boolean isMinimizedMode) {
        ...
        this.mDismissEndTarget = (SnapTarget) arrayList.get(arrayList.size() - 1);
        arrayList = this.mTargets;
        this.mMiddleTarget = (SnapTarget) arrayList.get(arrayList.size() / 2);
    ...
    private void calculateTargets(boolean isHorizontalDivision, int dockedSide) {
        ...
        int navBarSize = this.mInsets;
        navBarSize = isHorizontalDivision ? navBarSize.bottom : navBarSize.right;
    ...
    private void addFixedDivisionTargets(boolean isHorizontalDivision, int dividerMax) {
        int end;
        int start = this.mInsets;
        start = isHorizontalDivision ? start.top : start.left;
        if (isHorizontalDivision) {
            end = this.mDisplayHeight - this.mInsets.bottom;
        } else {
            end = this.mDisplayWidth - this.mInsets.right;
        }
        int size = (int) (this.mFixedRatio * ((float) (end - start)));
    ...
    private void addRatio16_9Targets(boolean isHorizontalDivision, int dividerMax) {
        ...
        int startOther = this.mInsets;
        startOther = isHorizontalDivision ? startOther.left : startOther.top;
        if (isHorizontalDivision) {
    ...
    public SnapTarget cycleNonDismissTarget(SnapTarget snapTarget, int increment) {
        ...
        ArrayList arrayList = this.mTargets;
        SnapTarget newTarget = (SnapTarget) arrayList.get(((arrayList.size() + index) + increment) % this.mTargets.size());
