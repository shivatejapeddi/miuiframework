    ...
    public static class StopInfo implements Runnable {
        ...
        public void run() {
            try {
                ...
                String str = TAG;
                IndentingPrintWriter pw = new IndentingPrintWriter(new LogWriter(5, str), "  ");
                pw.println("Bundle stats:");
                BaseBundle.dumpStats(pw, (BaseBundle) this.mState);
                pw.println("PersistableBundle stats:");
                BaseBundle.dumpStats(pw, (BaseBundle) this.mPersistentState);
                if (!(ex instanceof TransactionTooLargeException) || this.mActivity.packageInfo.getTargetSdkVersion() >= 24) {
                    throw ex.rethrowFromSystemServer();
                }
                Log.e(str, "App sent too much data in instance state, so it was ignored", ex);
            }
        }
    }
