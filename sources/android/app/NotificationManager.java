...
public class NotificationManager {
    ...
    public static class Policy implements Parcelable {
        ...
        private static final int[] SCREEN_ON_SUPPRESSED_EFFECTS = new int[]{2, 16, 32, 64, 256};
        ...
        public static boolean areAllVisualEffectsSuppressed(int effects) {
            int i = 0;
            while (true) {
                int effect = ALL_SUPPRESSED_EFFECTS;
                if (i >= effect.length) {
                    return true;
                }
                if ((effects & effect[i]) == 0) {
                    return false;
                }
                i++;
            }
        ...
        public static String priorityCategoriesToString(int priorityCategories) {
            ...
            int i = 0;
            while (true) {
                int priorityCategory = ALL_PRIORITY_CATEGORIES;
                if (i >= priorityCategory.length) {
                    break;
                }
                priorityCategory = priorityCategory[i];
    ...
    private Notification fixNotification(Notification notification) {
        ...
        if (this.mContext.getApplicationInfo().targetSdkVersion <= 22 || notification.getSmallIcon() != null) {
            ...
            return Builder.maybeCloneStrippedForDelivery(notification, ((ActivityManager) this.mContext.getSystemService(Context.ACTIVITY_SERVICE)).isLowRamDevice(), this.mContext);
        }
        StringBuilder stringBuilder = new StringBuilder();
    ...
    public Map<String, AutomaticZenRule> getAutomaticZenRules() {
        RemoteException e;
        INotificationManager service = getService();
        INotificationManager service2;
        try {
            List<ZenRule> rules = service.getZenRules();
            Map<String, AutomaticZenRule> ruleMap = new HashMap();
            Iterator it = rules.iterator();
            while (it.hasNext()) {
                ZenRule rule = (ZenRule) it.next();
                String str = rule.id;
                List<ZenRule> rules2 = rules;
                service2 = service;
                try {
                    Iterator it2 = it;
                    AutomaticZenRule automaticZenRule = r6;
                    AutomaticZenRule automaticZenRule2 = new AutomaticZenRule(rule.name, rule.component, rule.configurationActivity, rule.conditionId, rule.zenPolicy, zenModeToInterruptionFilter(rule.zenMode), rule.enabled, rule.creationTime);
    ...
    public void setNotificationPolicy(Policy policy) {
        checkRequired(PerfEventConstants.FIELD_SCHED_POLICY, policy);
        try {
