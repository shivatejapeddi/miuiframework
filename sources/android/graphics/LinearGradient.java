...
public class LinearGradient extends Shader {
    ...
    public LinearGradient(float x0, float y0, float x1, float y1, int[] colors, float[] positions, TileMode tile) {
        this(x0, y0, x1, y1, Shader.convertColors(colors), positions, tile, ColorSpace.get(Named.SRGB));
    }

    public LinearGradient(float x0, float y0, float x1, float y1, long[] colors, float[] positions, TileMode tile) {
        this(x0, y0, x1, y1, (long[]) colors.clone(), positions, tile, Shader.detectColorSpace(colors));
    }
