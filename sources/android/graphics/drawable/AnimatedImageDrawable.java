...
public class AnimatedImageDrawable extends Drawable implements Animatable2 {
    ...
    private void updateStateFromTypedArray(TypedArray a, int srcDensityOverride) throws XmlPullParserException {
        ...
        int i = srcDensityOverride;
        State oldState = this.mState;
        Resources r = a.getResources();
        int srcResId = typedArray.getResourceId(0, 0);
        String str = ": <animated-image> requires a valid 'src' attribute";
        if (srcResId != 0) {
            TypedValue value = new TypedValue();
            r.getValueForDensity(srcResId, i, value, true);
            if (i > 0 && value.density > 0 && value.density != 65535) {
                if (value.density == i) {
                    value.density = r.getDisplayMetrics().densityDpi;
                } else {
                    value.density = (value.density * r.getDisplayMetrics().densityDpi) / i;
                }
            }
            int density = 0;
            if (value.density == 0) {
            ...
            try {
                Drawable drawable = ImageDecoder.decodeDrawable(ImageDecoder.createSource(r, r.openRawResource(srcResId, value), density), -$$Lambda$AnimatedImageDrawable$Cgt3NliB7ZYUONyDd-eQGdYbEKc.INSTANCE);
                if (drawable instanceof AnimatedImageDrawable) {
                    Drawable drawable2 = this.mState.mRepeatCount;
        ...
        if (this.mState.mNativePtr == 0 && (this.mState.mThemeAttrs == null || this.mState.mThemeAttrs[0] == 0)) {
            ...
            stringBuilder3.append(a.getPositionDescription());
            stringBuilder3.append(str);
            throw new XmlPullParserException(stringBuilder3.toString());
        }
        this.mState.mAutoMirrored = typedArray.getBoolean(3, oldState.mAutoMirrored);
        int repeatCount = typedArray.getInt(1, -2);
        if (repeatCount != -2) {
    ...
    public AnimatedImageDrawable(long nativeImageDecoder, ImageDecoder decoder, int width, int height, long colorSpaceHandle, boolean extended, int srcDensity, int dstDensity, Rect cropRect, InputStream inputStream, AssetFileDescriptor afd) throws IOException {
        ...
        registry.registerNativeAllocation(state3, state3.mNativePtr);
    }
    ...
    public /* synthetic */ void lambda$postOnAnimationStart$1$AnimatedImageDrawable() {
        ...
        while (it.hasNext()) {
            ((AnimationCallback) it.next()).onAnimationStart(this);
        }
    }
