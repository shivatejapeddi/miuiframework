...
public class LayerDrawable extends Drawable implements Callback {
    ...
    static class ChildDrawable {
        ...
        private void applyDensityScaling(int sourceDensity, int targetDensity) {
            ...
            i = this.mInsetE;
            if (i != Integer.MIN_VALUE) {
                this.mInsetE = Drawable.scaleFromDensity(i, sourceDensity, targetDensity, false);
            }
            i = this.mWidth;
            if (i > 0) {
    ...
    static class LayerState extends ConstantState {
        ...
        private void applyDensityScaling(int sourceDensity, int targetDensity) {
            ...
            i = this.mPaddingTop;
            if (i > 0) {
                this.mPaddingTop = Drawable.scaleFromDensity(i, sourceDensity, targetDensity, false);
            }
            i = this.mPaddingRight;
            if (i > 0) {
                this.mPaddingRight = Drawable.scaleFromDensity(i, sourceDensity, targetDensity, false);
            }
            i = this.mPaddingBottom;
            if (i > 0) {
                this.mPaddingBottom = Drawable.scaleFromDensity(i, sourceDensity, targetDensity, false);
            }
            i = this.mPaddingStart;
            if (i > 0) {
                this.mPaddingStart = Drawable.scaleFromDensity(i, sourceDensity, targetDensity, false);
            }
            i = this.mPaddingEnd;
            if (i > 0) {
                this.mPaddingEnd = Drawable.scaleFromDensity(i, sourceDensity, targetDensity, false);
            }
        }
        ...
        if (layers != null) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (child != null) {
                    ...
                    layerState.mChildrenChangingConfigurations |= child.getChangingConfigurations();
                }
            }
            LayerState layerState2 = this.mLayerState;
            layerState2.mNumChildren = length;
            layerState2.mChildren = r;
            ensurePadding();
            refreshPadding();
            return;
        }
    ...
    private void updateLayerBoundsInternal(Rect bounds) {
        ...
        while (i < count) {
            ...
            if (d == null) {
                ...
                isLayoutRtl2 = isLayoutRtl;
                int insetL = count == Integer.MIN_VALUE ? r.mInsetL : count;
                int insetR = insetRtlR == Integer.MIN_VALUE ? r.mInsetR : insetRtlR;
                Rect container = this.mTmpContainer;
                i2 = paddingT;
                container.set((rect.left + insetL) + paddingL, (rect.top + insetT) + paddingT, (rect.right - insetR) - paddingR, (rect.bottom - insetB) - paddingB);
                ...
                insetB = insetRtlR < 0 ? paddingT : insetRtlR;
                Drawable d2 = d;
                Gravity.apply(gravity, insetB, layerH >= false ? isLayoutRtl : layerH, container, outRect, layoutDirection);
