...
public final class SharedMemory implements Parcelable, Closeable {
    ...
    private SharedMemory(FileDescriptor fd) {
        if (fd == null) {
            ...
            int i = this.mSize;
            if (i > 0) {
                this.mMemoryRegistration = new MemoryRegistration(i, null);
                FileDescriptor fileDescriptor = this.mFileDescriptor;
                this.mCleaner = Cleaner.create(fileDescriptor, new Closer(fileDescriptor, this.mMemoryRegistration, null));
                return;
            }
    ...
    public ByteBuffer map(int prot, int offset, int length) throws ErrnoException {
        ...
        if (i < 0) {
            ...
            return new DirectByteBuffer(length, address, this.mFileDescriptor, new Unmapper(address, length, this.mMemoryRegistration.acquire(), null), (prot & OsConstants.PROT_WRITE) == 0);
        } else {
