...
public abstract class BaseLooper implements ILooperMonitorable {
    ...
    static class MessageMonitorInfo implements Parcelable, Cloneable {
        ...
        public void markDispatch(Message msg) {
            ...
            BaseLooper.nGetThreadDelay(delays);
            ...
            try {
                ...
                if (!TextUtils.isEmpty(this.targetName)) {
                    ...
                    if (this.planCurrentTime != 0) {
                        ...
                        stringBuilder.append(BaseLooper.formatCurrentTime(this.planCurrentTime));
                        sb.append(stringBuilder.toString());
                    }
                    stringBuilder = new StringBuilder();
                ...
                Log.e(BaseLooper.TAG, sb.toString());
                return result;
            }
        }
    ...
    public void enableMonitor(boolean enable) {
        ...
        } else if (PerfSupervisionSettings.isSupervisionOn()) {
    ...
    /* Access modifiers changed, original: 0000 */
    public void enableAnrMonitor(boolean enable) {
        this.mEnableAnrMonitor = enable;
        this.mEnableMonitorMessage = enable;
        updateCallbackIfNeed(enable);
    }
    ...
    private void dumpLtMessageHistory(java.io.PrintWriter r10, java.lang.String r11, long r12) {
        ...
        r2 = r9.toString();	 Catch:{ all -> 0x006d }
        r1.append(r2);	 Catch:{ all -> 0x006d }
        r2 = ":";
        r1.append(r2);	 Catch:{ all -> 0x006d }
    ...
    L_0x006d:
        ...
        throw r1;
        */
        throw new UnsupportedOperationException("Method not decompiled: android.os.BaseLooper.dumpLtMessageHistory(java.io.PrintWriter, java.lang.String, long):void");
    }
    ...
    private void dumpMessageQueue(PrintWriter pw, String prefix) {
        MessageQueue queue = getQueue();
        if (queue != null) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(prefix);
            stringBuilder.append("MessageQueue on ");
            stringBuilder.append(toString());
            stringBuilder.append(":");
            pw.println(stringBuilder.toString());
            queue.dump(new PrintWriterPrinter(pw), prefix, null);
        }
    }

    private void dumpInternal(PrintWriter writer, String prefix, long duration, boolean dumpQueue) {
        if (this.mEnableMonitorMessage) {
            dumpRunningMessage(writer, prefix);
            dumpLtMessageHistory(writer, prefix, duration);
        }
        if (dumpQueue) {
            dumpMessageQueue(writer, prefix);
        }
    }
    ...
    public void dumpAllLoopers(PrintWriter pw, String prefix) {
        if (this.mEnableMonitorMessage) {
            synchronized (sCallbacksLock) {
                ...
                while (it.hasNext()) {
                    BaseLooper looper = (BaseLooper) ((WeakReference) it.next()).get();
                    if (looper != null) {
                        looper.dumpLtMessageHistory(pw, prefix, AlarmManager.INTERVAL_HALF_HOUR);
                    }
                }
            }
        }
    ...
    public String dumpAll(String prefix, long duration) {
        ...
        stringBuilder.append(toString());
        writer.println(stringBuilder.toString());
        dumpInternal(writer, prefix, duration, false);
        writer.flush();
        stringBuilder = new StringBuilder();
    ...
    static String messageToString(Message msg) {
        ...
        try {
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("getMessageString failed ! ");
