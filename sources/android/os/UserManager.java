...
public class UserManager {
    ...
    public static boolean supportsMultipleUsers() {
        if (getMaxSupportedUsers() > 1) {
            if (SystemProperties.getBoolean("fw.show_multiuserui", Resources.getSystem().getBoolean(R.bool.config_enableMultiUserUI))) {
                return true;
            }
        }
        return false;
    ...
    public int getUserSwitchability() {
        boolean z = false;
        if (Global.getInt(this.mContext.getContentResolver(), Global.ALLOW_USER_SWITCHING_WHEN_SYSTEM_USER_LOCKED, 0) != 0) {
            z = true;
        }
        boolean allowUserSwitchingWhenSystemUserLocked = z;
        z = isUserUnlocked(UserHandle.SYSTEM);
        int flags = 0;
        ...
        if (allowUserSwitchingWhenSystemUserLocked || z) {
            return flags;
        }
        return flags | 4;
    }
    ...
    public boolean isUserSwitcherEnabled() {
        ...
        if (!(Global.getInt(this.mContext.getContentResolver(), Global.USER_SWITCHER_ENABLED, 1) != 0)) {
            return false;
        }
        List<UserInfo> users = getUsers(true);
        if (users == null) {
