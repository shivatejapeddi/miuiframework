...
public class DrmManagerClient implements AutoCloseable {
    ...
    private class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (i == 1001) {
                ...
                Log.e(DrmManagerClient.TAG, stringBuilder.toString());
                return;
            } else {
                DrmInfo drmInfo = msg.obj;
                DrmInfoStatus status = DrmManagerClient.this;
                status = status._processDrmInfo(status.mUniqueId, drmInfo);
    ...
    public static void notify(Object thisReference, int uniqueId, int infoType, String message) {
        ...
        if (instance != null) {
            Message m = instance.mInfoHandler;
            if (m != null) {
                instance.mInfoHandler.sendMessage(m.obtainMessage(1, uniqueId, infoType, message));
            }
        }
    }
    ...
    public boolean canHandle(String path, String mimeType) {
        String str = "";
        if ((path != null && !path.equals(str)) || (mimeType != null && !mimeType.equals(str))) {
    ...
    public int processDrmInfo(DrmInfo drmInfo) {
        ...
        Message msg = this.mEventHandler;
        if (msg == null) {
            return ERROR_UNKNOWN;
        }
        return this.mEventHandler.sendMessage(msg.obtainMessage(1002, drmInfo)) ? 0 : ERROR_UNKNOWN;
    }
    ...
    public int removeAllRights() {
        ...
        return this.mEventHandler.sendMessage(msg.obtainMessage(1001)) ? 0 : ERROR_UNKNOWN;
    }
    ...
    private String convertUriToPath(Uri uri) {
        ...
        if (scheme.equals("content")) {
            ...
            Cursor cursor = null;
            try {
                cursor = this.mContext.getContentResolver().query(uri, new String[]{pathIndex}, null, null, null);
                if (cursor == null || cursor.getCount() == 0 || !cursor.moveToFirst()) {
                    throw new IllegalArgumentException("Given Uri could not be found in media store");
                }
                String path = cursor.getString(cursor.getColumnIndexOrThrow(pathIndex));
                cursor.close();
                return path;
