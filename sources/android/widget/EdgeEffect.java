...
public class EdgeEffect {
    ...
    public void onPull(float deltaDistance, float displacement) {
        ...
        if (this.mState != 4 || ((float) (now - this.mStartTime)) >= this.mDuration) {
            ...
            this.mGlowAlpha = min;
            min = this.mPullDistance;
            if (min == 0.0f) {
                this.mGlowScaleYStart = 0.0f;
                this.mGlowScaleY = 0.0f;
            } else {
                min = (float) (Math.max(0.0d, (1.0d - (1.0d / Math.sqrt((double) (Math.abs(min) * ((float) this.mBounds.height()))))) - 0.3d) / 0.7d);
    ...
    private void update() {
        ...
        f = this.mGlowScaleYStart;
        this.mGlowScaleY = f + ((this.mGlowScaleYFinish - f) * interp);
