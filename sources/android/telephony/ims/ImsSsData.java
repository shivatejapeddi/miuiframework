...
public final class ImsSsData implements Parcelable {
    ...
    public int[] getSuppServiceInfoCompat() {
        ...
        iArr = new int[2];
        List list = this.mImsSsInfo;
        if (list == null || list.size() == 0) {
            Rlog.e(TAG, "getSuppServiceInfoCompat: Could not parse mImsSsInfo, returning empty int[]");
            return iArr;
        } else if (isTypeClir()) {
            iArr[0] = ((ImsSsInfo) this.mImsSsInfo.get(0)).getClirOutgoingState();
            iArr[1] = ((ImsSsInfo) this.mImsSsInfo.get(0)).getClirInterrogationStatus();
            return iArr;
        } else {
            if (isTypeColr()) {
                iArr[0] = ((ImsSsInfo) this.mImsSsInfo.get(0)).getProvisionStatus();
            }
            iArr[0] = ((ImsSsInfo) this.mImsSsInfo.get(0)).getStatus();
            iArr[1] = ((ImsSsInfo) this.mImsSsInfo.get(0)).getProvisionStatus();
            return iArr;
        }
    }
