...
public class MbmsGroupCallSession implements AutoCloseable {
    ...
    public void close() {
        try {
            IMbmsGroupCallService groupCallService = (IMbmsGroupCallService) this.mService.get();
    ...
    private int bindAndInitialize() {
        return MbmsUtils.startBinding(this.mContext, MBMS_GROUP_CALL_SERVICE_ACTION, new ServiceConnection() {
            public void onServiceConnected(ComponentName name, IBinder service) {
                String str = MbmsGroupCallSession.LOG_TAG;
                IMbmsGroupCallService groupCallService = Stub.asInterface(service);
                try {
                    str = groupCallService.initialize(MbmsGroupCallSession.this.mInternalCallback, MbmsGroupCallSession.this.mSubscriptionId);
                    if (str == -1) {
                        MbmsGroupCallSession.this.close();
                        throw new IllegalStateException("Middleware must not return an unknown error code");
                    } else if (str != null) {
                        MbmsGroupCallSession.this.mInternalCallback.onError(str, "Error returned during initialization");
                        MbmsGroupCallSession.sIsInitialized.set(false);
                    } else {
                        try {
                    ...
                    Log.e(str, "Service died before initialization");
                    MbmsGroupCallSession.this.mInternalCallback.onError(103, e2.toString());
                    MbmsGroupCallSession.sIsInitialized.set(false);
                } catch (RuntimeException e3) {
                    Log.e(str, "Runtime exception during initialization");
