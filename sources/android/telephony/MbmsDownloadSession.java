...
public class MbmsDownloadSession implements AutoCloseable {
    ...
    private void writeDownloadRequestToken(DownloadRequest request) {
        String str = "Failed to create download token for request ";
        ...
        try {
            if (!token.createNewFile()) {
                ...
                stringBuilder2.append(". Token location is ");
            ...
            stringBuilder3.append(" due to IOException ");
            stringBuilder3.append(e);
            stringBuilder3.append(". Attempted to write to ");
            stringBuilder3.append(token.getPath());
    ...
    private void checkDownloadRequestDestination(DownloadRequest request) {
        ...
        if (downloadRequestDestination.isDirectory()) {
            File embmsTempFileDir = MbmsTempFileProvider.getEmbmsTempFileDir(this.mContext);
            String str = DESTINATION_SANITY_CHECK_FILE_NAME;
            File testFile = new File(embmsTempFileDir, str);
            embmsTempFileDir = new File(downloadRequestDestination, str);
            try {
                if (!testFile.exists()) {
                    testFile.createNewFile();
                }
                if (testFile.renameTo(embmsTempFileDir)) {
                    testFile.delete();
                    embmsTempFileDir.delete();
                    return;
                }
