...
public class MediaSource extends Filter {
    ...
    private OnVideoSizeChangedListener onVideoSizeChangedListener = new OnVideoSizeChangedListener() {
        public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
            if (MediaSource.this.mLogVerbose) {
                ...
                Log.v(MediaSource.TAG, stringBuilder.toString());
            }
    ...
    public void open(FilterContext context) {
        if (this.mLogVerbose) {
            ...
            if (this.mSelectedIsUrl) {
                ...
                Log.v(str, "Current source is Asset!");
            }
        }
    ...
    public void process(FilterContext context) {
        ...
        if (this.mMediaPlayer != null) {
            String str2 = "video";
            if (this.mCompleted) {
                closeOutputPort(str2);
                return;
            }
            if (!this.mPlaying) {
                if (this.mLogVerbose) {
                    Log.v(str, "Waiting for preparation to complete");
                }
                int waitCount = 0;
                while (true) {
                    if (this.mGotSize && this.mPrepared) {
                        if (this.mLogVerbose) {
                            Log.v(str, "Starting playback");
                        }
            ...
            if (!(this.mPaused && this.mPlaying)) {
                if (this.mWaitForNewFrame) {
                    if (this.mLogVerbose) {
                        Log.v(str, "Waiting for new frame");
                    }
                    int waitCount2 = 0;
    ...
    public void fieldPortValueUpdated(String name, FilterContext context) {
        ...
        String str = TAG;
        if (z) {
            Log.v(str, "Parameter update");
        }
        String str2 = "Opening new source URL";
        if (name.equals("sourceUrl")) {
            if (isOpen()) {
                if (this.mLogVerbose) {
                    Log.v(str, str2);
                }
                z = this.mSelectedIsUrl;
                if (z) {
                    setupMediaPlayer(z);
                }
            }
            ...
            if (isOpen()) {
                if (this.mLogVerbose) {
                    Log.v(str, "Opening new source FD");
                }
                z = this.mSelectedIsUrl;
                if (!z) {
                    setupMediaPlayer(z);
            ...
            if (isOpen()) {
                if (this.mSelectedIsUrl) {
                    ...
                    Log.v(str, "Opening new source Asset");
                }
