...
import android.provider.Settings.Config;
...
public final class DeviceConfig {
    ...
    public static class Properties {
        ...
        public String getString(String name, String defaultValue) {
            ...
            String value = (String) this.mMap.get(name);
    ...
    public static String getProperty(String namespace, String name) {
        return Config.getString(ActivityThread.currentApplication().getContentResolver(), createCompositeName(namespace, name));
    }
    ...
    public static boolean setProperty(String namespace, String name, String value, boolean makeDefault) {
        return Config.putString(ActivityThread.currentApplication().getContentResolver(), createCompositeName(namespace, name), value, makeDefault);
    }

    @SystemApi
    public static void resetToDefaults(int resetMode, String namespace) {
        Config.resetToDefaults(ActivityThread.currentApplication().getContentResolver(), resetMode, namespace);
    }
    ...
    public static void addOnPropertyChangedListener(String namespace, Executor executor, OnPropertyChangedListener onPropertyChangedListener) {
        ...
        synchronized (sLock) {
            Pair<String, Executor> oldNamespace = (Pair) sSingleListeners.get(onPropertyChangedListener);
            if (oldNamespace == null) {
                ...
                decrementNamespace((String) ((Pair) sSingleListeners.get(onPropertyChangedListener)).first);
                sSingleListeners.put(onPropertyChangedListener, new Pair(namespace, executor));
                incrementNamespace(namespace);
            }
        }
    ...
    public static void addOnPropertiesChangedListener(String namespace, Executor executor, OnPropertiesChangedListener onPropertiesChangedListener) {
        enforceReadPermission(ActivityThread.currentApplication().getApplicationContext(), namespace);
        synchronized (sLock) {
            Pair<String, Executor> oldNamespace = (Pair) sListeners.get(onPropertiesChangedListener);
            if (oldNamespace == null) {
                ...
                decrementNamespace((String) ((Pair) sListeners.get(onPropertiesChangedListener)).first);
                sListeners.put(onPropertiesChangedListener, new Pair(namespace, executor));
                incrementNamespace(namespace);
            }
        }
    ...
    private static void incrementNamespace(String namespace) {
        Preconditions.checkNotNull(namespace);
        Pair<ContentObserver, Integer> namespaceCount = (Pair) sNamespaces.get(namespace);
        ...
        sNamespaces.put(namespace, new Pair(contentObserver, Integer.valueOf(1)));
    }
    ...
    private static void handleChange(Uri uri) {
        ...
        final String namespace = (String) pathSegments.get(1);
        final String name = (String) pathSegments.get(2);
        try {
            ...
            synchronized (sLock) {
                ...
                for (i = 0; i < sListeners.size(); i++) {
                    if (namespace.equals(((Pair) sListeners.valueAt(i)).first)) {
                        final int j = i;
                        ((Executor) ((Pair) sListeners.valueAt(i)).second).execute(new Runnable() {
                            public void run() {
                ...
                while (i2 < sSingleListeners.size()) {
                    if (namespace.equals(((Pair) sSingleListeners.valueAt(i2)).first)) {
                        i = i2;
                        ((Executor) ((Pair) sSingleListeners.valueAt(i2)).second).execute(new Runnable() {
                            public void run() {
                                ((OnPropertyChangedListener) DeviceConfig.sSingleListeners.keyAt(i)).onPropertyChanged(namespace, name, value);
                            }
                        });
                    }
                    i2++;
    ...
    public static void enforceReadPermission(Context context, String namespace) {
        if (context.checkCallingOrSelfPermission(permission.READ_DEVICE_CONFIG) != 0 && !PUBLIC_NAMESPACES.contains(namespace)) {
