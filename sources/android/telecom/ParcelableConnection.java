...
public final class ParcelableConnection implements Parcelable {
    public static final Creator<ParcelableConnection> CREATOR = new Creator<ParcelableConnection>() {
        public ParcelableConnection createFromParcel(Parcel source) {
            ...
            List list = conferenceableConnectionIds;
            parcel.readStringList(conferenceableConnectionIds);
            return new ParcelableConnection(phoneAccount, state, capabilities, source.readInt(), source.readInt(), address, addressPresentation, callerDisplayName, callerDisplayNamePresentation, videoCallProvider, videoState, ringbackRequested, audioModeIsVoip, readLong, source.readLong(), statusHints, disconnectCause, list, Bundle.setDefusable(parcel.readBundle(classLoader), true), source.readString(), source.readInt());
        }
