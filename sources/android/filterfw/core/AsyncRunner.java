...
public class AsyncRunner extends GraphRunner {
    ...
    private class AsyncRunnerTask extends AsyncTask<SyncRunner, Void, RunnerResult> {
            ...
            try {
                if (runner.length <= 1) {
                    ...
                    if (AsyncRunner.this.mLogVerbose) {
                        Log.v(str, "Starting background graph processing.");
                    }
                    AsyncRunner.this.activateGlContext();
                    if (AsyncRunner.this.mLogVerbose) {
                        Log.v(str, "Preparing filter graph for processing.");
                    }
                    runner[0].beginProcessing();
                    if (AsyncRunner.this.mLogVerbose) {
                        Log.v(str, "Running graph.");
                    }
                    result.status = 1;
                    ...
                    if (AsyncRunner.this.mLogVerbose) {
                        Log.v(str, "Done with background graph processing.");
                    }
                    return result;
                }
        ...
        public void onPostExecute(RunnerResult result) {
            boolean access$100 = AsyncRunner.this.mLogVerbose;
            String str = TAG;
            if (access$100) {
                Log.v(str, "Starting post-execute.");
            }
            ...
            if (AsyncRunner.this.mDoneListener != null) {
                if (AsyncRunner.this.mLogVerbose) {
                    Log.v(str, "Calling graph done callback.");
                }
                AsyncRunner.this.mDoneListener.onRunnerDone(result.status);
            }
            if (AsyncRunner.this.mLogVerbose) {
                Log.v(str, "Completed post-execute.");
            }
        }
    }
    ...
    public FilterGraph getGraph() {
        ...
        return syncRunner != null ? syncRunner.getGraph() : null;
    }
