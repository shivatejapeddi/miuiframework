    ...
    public static abstract class Stub extends Binder implements ISession {
        ...
        private static class Proxy implements ISession {
            ...
            public ISessionController getController() throws RemoteException {
                ...
                try {
                    _data.writeInterfaceToken(Stub.DESCRIPTOR);
                    ISessionController iSessionController = 2;
                    if (!this.mRemote.transact(2, _data, _reply, 0)) {
                        iSessionController = Stub.getDefaultImpl();
                        if (iSessionController != 0) {
                            iSessionController = Stub.getDefaultImpl().getController();
                            return iSessionController;
                        }
                    }
                    _reply.readException();
                    iSessionController = android.media.session.ISessionController.Stub.asInterface(_reply.readStrongBinder());
                    ISessionController _result = iSessionController;
                    _reply.recycle();
                    _data.recycle();
                    return _result;
                } finally {
