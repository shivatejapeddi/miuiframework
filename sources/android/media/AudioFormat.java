...
public final class AudioFormat implements Parcelable {
    ...
    public static class Builder {
        ...
        public AudioFormat build() {
            return new AudioFormat(this.mPropertySetMask, this.mEncoding, this.mSampleRate, this.mChannelMask, this.mChannelIndexMask, null);
        }
    ...
    /* synthetic */ AudioFormat(int x0, int x1, int x2, int x3, int x4, AnonymousClass1 x5) {
        this(x0, x1, x2, x3, x4);
    }

    /* synthetic */ AudioFormat(Parcel x0, AnonymousClass1 x1) {
        this(x0);
    }
    ...
    private AudioFormat(int propertySetMask, int encoding, int sampleRate, int channelMask, int channelIndexMask) {
        ...
        if ((propertySetMask & 8) != 0) {
            i = channelIndexMask;
        }
        this.mChannelIndexMask = i;
        int channelIndexCount = Integer.bitCount(getChannelIndexMask());
        i = channelCountFromOutChannelMask(getChannelMask());
        if (i == 0) {
            i = channelIndexCount;
        } else if (!(i == channelIndexCount || channelIndexCount == 0)) {
            i = 0;
        }
        this.mChannelCount = i;
        int frameSizeInBytes = 1;
        try {
            frameSizeInBytes = getBytesPerSample(this.mEncoding) * i;
