...
public final class AudioProductStrategy implements Parcelable {
    public static final Creator<AudioProductStrategy> CREATOR = new Creator<AudioProductStrategy>() {
        public AudioProductStrategy createFromParcel(Parcel in) {
            ...
            for (int index = 0; index < nbAttributesGroups; index++) {
                aag[index] = (AudioAttributesGroup) AudioAttributesGroup.CREATOR.createFromParcel(in);
            }
            return new AudioProductStrategy(name, id, aag, null);
        }
    ...
    public static final AudioAttributes sDefaultAttributes = new Builder().setCapturePreset(0).build();
    ...
    private static final class AudioAttributesGroup implements Parcelable {
        public static final Creator<AudioAttributesGroup> CREATOR = new Creator<AudioAttributesGroup>() {
            public AudioAttributesGroup createFromParcel(Parcel in) {
                ...
                for (int index = 0; index < nbAttributes; index++) {
                    aa[index] = (AudioAttributes) AudioAttributes.CREATOR.createFromParcel(in);
                }
    ...
    /* synthetic */ AudioProductStrategy(String x0, int x1, AudioAttributesGroup[] x2, AnonymousClass1 x3) {
        this(x0, x1, x2);
    }
    ...
    public static int getLegacyStreamTypeForStrategyWithAudioAttributes(AudioAttributes audioAttributes) {
        ...
        for (AudioProductStrategy productStrategy : getAudioProductStrategies()) {
            if (productStrategy.supportsAudioAttributes(audioAttributes)) {
                ...
                stringBuilder.append(productStrategy.getId());
    ...
    private static boolean attributesMatches(AudioAttributes refAttr, AudioAttributes attr) {
        Preconditions.checkNotNull(refAttr, "refAttr must not be null");
        Preconditions.checkNotNull(attr, "attr must not be null");
        CharSequence cliFormattedTags = ";";
        String refFormattedTags = TextUtils.join(cliFormattedTags, refAttr.getTags());
        String cliFormattedTags2 = TextUtils.join(cliFormattedTags, attr.getTags());
        boolean z = false;
        if (refAttr.equals(sDefaultAttributes)) {
            return false;
        }
        if ((refAttr.getUsage() == 0 || attr.getUsage() == refAttr.getUsage()) && ((refAttr.getContentType() == 0 || attr.getContentType() == refAttr.getContentType()) && ((refAttr.getAllFlags() == 0 || (attr.getAllFlags() != 0 && (attr.getAllFlags() & refAttr.getAllFlags()) == refAttr.getAllFlags())) && (refFormattedTags.length() == 0 || refFormattedTags.equals(cliFormattedTags2))))) {
            z = true;
        }
        return z;
    }
