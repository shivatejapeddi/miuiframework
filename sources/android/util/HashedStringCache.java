...
public class HashedStringCache {
    ...
    public HashResult hashString(Context context, String tag, String clearText, int saltExpirationDays) {
        ...
        String hashText = (String) this.mHashes.get(clearText);
        if (hashText != null) {
        ...
        hashText = Base64.encodeToString(bytes, 0, Math.min(8, bytes.length), 3);
    ...
    private void populateSaltValues(Context context, String tag, int saltExpirationDays) {
        synchronized (this.mPreferenceLock) {
            ...
            long saltDate = this.mSharedPreferences;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(tag);
            stringBuilder.append(HASH_SALT_DATE);
            boolean needsNewSalt = checkNeedsNewSalt(tag, saltExpirationDays, saltDate.getLong(stringBuilder.toString(), 0));
            if (needsNewSalt) {
                this.mHashes.evictAll();
            }
            if (this.mSalt == null || needsNewSalt) {
                String saltString = this.mSharedPreferences;
                StringBuilder stringBuilder2 = new StringBuilder();
                stringBuilder2.append(tag);
                stringBuilder2.append(HASH_SALT);
                saltString = saltString.getString(stringBuilder2.toString(), null);
                SharedPreferences sharedPreferences = this.mSharedPreferences;
                StringBuilder stringBuilder3 = new StringBuilder();
                stringBuilder3.append(tag);
                stringBuilder3.append(HASH_SALT_GEN);
                this.mSaltGen = sharedPreferences.getInt(stringBuilder3.toString(), 0);
                if (saltString == null || needsNewSalt) {
                    ...
                    stringBuilder4.append(HASH_SALT);
                    edit = edit.putString(stringBuilder4.toString(), saltString);
                    stringBuilder4 = new StringBuilder();
                    stringBuilder4.append(tag);
                    stringBuilder4.append(HASH_SALT_GEN);
                    edit = edit.putInt(stringBuilder4.toString(), this.mSaltGen);
                    stringBuilder4 = new StringBuilder();
                    stringBuilder4.append(tag);
                    stringBuilder4.append(HASH_SALT_DATE);
                    edit.putLong(stringBuilder4.toString(), System.currentTimeMillis()).apply();
                }
