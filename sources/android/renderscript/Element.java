...
public class Element extends BaseObj {
        ...
        if (this.mElements != null) {
            ...
            for (ct = 0; ct < fieldCount; ct++) {
                if (this.mElementNames[ct].charAt(0) != '#') {
                    int ctNoPadding2 = ctNoPadding + 1;
    ...
    public void updateFromNative() {
        ...
        if (numSubElements > 0) {
            ...
            for (int i3 = 0; i3 < numSubElements; i3++) {
                this.mElements[i3] = new Element(subElementIds[i3], this.mRS);
                this.mElements[i3].updateFromNative();
                int[] iArr = this.mOffsetInBytes;
                i = this.mSize;
                iArr[i3] = i;
                this.mSize = i + (this.mElements[i3].mSize * this.mArraySizes[i3]);
            }
        }
        updateVisibleSubElements();
    }
    ...
    public static Element createPixel(RenderScript rs, DataType dt, DataKind dk) {
        if (dk != DataKind.PIXEL_L && dk != DataKind.PIXEL_A && dk != DataKind.PIXEL_LA && dk != DataKind.PIXEL_RGB && dk != DataKind.PIXEL_RGBA && dk != DataKind.PIXEL_DEPTH && dk != DataKind.PIXEL_YUV) {
            ...
            String str = "Bad kind and type combo";
