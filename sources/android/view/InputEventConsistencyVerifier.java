...
public final class InputEventConsistencyVerifier {
        ...
        if (startEvent(event, nestingLevel, EVENT_TYPE_TOUCH)) {
            ...
            try {
                ...
                if ((source & 2) == 0) {
                    ...
                    String str = ".";
                    String str2 = " but the pointer count is ";
                    int id;
                    int idBit;
                    ...
                    if (expectedPointerCount == 5) {
                        ...
                        this.mTouchEventStreamIsTainted = true;
                        ensureHistorySizeIsZeroForThisAction(event);
                    } else if (expectedPointerCount == 6) {
                        if (actionIndex >= 0) {
                            if (actionIndex < pointerCount) {
                                id = event.getPointerId(actionIndex);
                                idBit = 1 << id;
                        ...
                        stringBuilder4 = new StringBuilder();
                        stringBuilder4.append("Invalid action ");
                        stringBuilder4.append(MotionEvent.actionToString(action));
                        stringBuilder4.append(" for touch event.");
                        problem(stringBuilder4.toString());
                    }
                } else {
                    if (this.mTouchEventStreamPointers != 0) {
    ...
    public void onGenericMotionEvent(MotionEvent event, int nestingLevel) {
        if (startEvent(event, nestingLevel, EVENT_TYPE_GENERIC_MOTION)) {
            try {
                ...
                if ((source & 2) != 0) {
                    String str = "Reported button state differs from expected button state based on press and release events. Is 0x%08x but expected 0x%08x.";
                    ...
                    switch (action) {
                        ...
                        case 11:
                            ...
                            if (this.mButtonsPressed != buttonState) {
                                problem(String.format(str, new Object[]{Integer.valueOf(buttonState), Integer.valueOf(this.mButtonsPressed)}));
                                break;
                            }
                            break;
                        case 12:
    ...
    private void finishEvent() {
        ...
        if (inputEventArr[i] != null) {
            inputEventArr[i].recycle();
        }
