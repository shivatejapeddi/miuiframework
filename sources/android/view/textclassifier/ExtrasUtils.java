...
public final class ExtrasUtils {
    ...
    static Bundle createForeignLanguageExtra(String language, float score, int modelVersion) {
        ...
        stringBuilder.append("langId_v");
        stringBuilder.append(modelVersion);
        bundle.putString(MODEL_NAME, stringBuilder.toString());
        return bundle;
    }
    ...
    public static ULocale getTopLanguage(Intent intent) {
        ...
        Bundle tcBundle = intent.getBundleExtra(TextClassifier.EXTRA_FROM_TEXT_CLASSIFIER);
        if (tcBundle == null) {
            return null;
        }
        Bundle textLanguagesExtra = tcBundle.getBundle(TEXT_LANGUAGES);
    ...
    public static RemoteAction findAction(TextClassification classification, String intentAction) {
        ...
        ArrayList<Intent> actionIntents = getActionsIntents(classification);
        if (actionIntents != null) {
            int size = actionIntents.size();
            for (int i = 0; i < size; i++) {
                Intent intent = (Intent) actionIntents.get(i);
                if (intent != null && intentAction.equals(intent.getAction())) {
                    return (RemoteAction) classification.getActions().get(i);
                }
            }
        }
        return null;
    ...
    public static void putEntities(Bundle container, ClassificationResult[] classifications) {
        if (!ArrayUtils.isEmpty((Object[]) classifications)) {
    ...
    public static boolean isSerializedEntityDataEnabled(Request request) {
