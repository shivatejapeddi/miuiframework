...
public class GridLayoutAnimationController extends LayoutAnimationController {
    ...
    private int getTransformedColumnIndex(AnimationParameters params) {
        int order = getOrder();
        if (order == 1) {
            order = (params.columnsCount - 1) - params.column;
        } else if (order != 2) {
            order = params.column;
        } else {
            ...
            order = (int) (((float) params.columnsCount) * this.mRandomizer.nextFloat());
        }
        if ((this.mDirection & 1) == 1) {
            return (params.columnsCount - 1) - order;
        }
        return order;
    }
    ...
    private int getTransformedRowIndex(AnimationParameters params) {
        int order = getOrder();
        if (order == 1) {
            order = (params.rowsCount - 1) - params.row;
        } else if (order != 2) {
            order = params.row;
        } else {
            if (this.mRandomizer == null) {
            ...
            order = (int) (((float) params.rowsCount) * this.mRandomizer.nextFloat());
        }
        if ((this.mDirection & 2) == 2) {
            return (params.rowsCount - 1) - order;
        }
        return order;
    }
}
