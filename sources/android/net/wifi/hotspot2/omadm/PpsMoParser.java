...
public final class PpsMoParser {
    ...
    public static PasspointConfiguration parseMoText(String xmlString) {
        try {
            ...
            String str = TAG;
            if (tag != TAG_MANAGEMENT_TREE) {
                Log.e(str, "Root is not a MgmtTree");
                return null;
            }
            tag = null;
            PasspointConfiguration config = null;
            for (XMLNode child : root.getChildren()) {
                String tag2 = child.getTag();
                Object obj = -1;
                int hashCode = tag2.hashCode();
                if (hashCode != -1736120495) {
                    if (hashCode == 2433570 && tag2.equals(TAG_NODE)) {
                        obj = 1;
                    }
                } else if (tag2.equals(TAG_VER_DTD)) {
                    obj = null;
                }
                if (obj != null) {
                    if (obj != 1) {
                        ...
                        Log.e(str, "Unexpected multiple Node element under MgmtTree");
                        return null;
                    } else {
                        try {
                    ...
                    Log.e(str, "Duplicate VerDTD element");
                    return null;
                } else {
                    tag = child.getText();
                }
    ...
    private static void parseEAPMethod(PPSNode node, UserCredential userCred) throws ParsingException {
        ...
        for (PPSNode child : node.getChildren()) {
            ...
            switch (obj) {
                ...
                default:
                    stringBuilder = new StringBuilder();
                    stringBuilder.append("Unknown node under EAPMethod: ");
                    stringBuilder.append(child.getName());
                    throw new ParsingException(stringBuilder.toString());
            }
        }
    ...
    private static UpdateParameter parseUpdateParameter(PPSNode node) throws ParsingException {
        ...
        for (PPSNode child : node.getChildren()) {
            ...
            switch (obj) {
                ...
                default:
                    stringBuilder = new StringBuilder();
                    stringBuilder.append("Unknown node under Update Parameters: ");
                    stringBuilder.append(child.getName());
                    throw new ParsingException(stringBuilder.toString());
            }
        }
