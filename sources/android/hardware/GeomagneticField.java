...
public class GeomagneticField {
            ...
            while (n <= maxN) {
                ...
                while (m <= n) {
                    ...
                    if (n == m) {
                        ...
                        fArr = this.mP;
                        fArr[n][m] = fArr[n - 1][m] * cos;
                        fArr2 = this.mPDeriv;
                        fArr2[n][m] = ((-sin) * fArr[n - 1][m]) + (fArr2[n - 1][m] * cos);
                    } else {
                        float k = ((float) (((n - 1) * (n - 1)) - (m * m))) / ((float) (((n * 2) - 1) * ((n * 2) - 3)));
                        fArr2 = this.mP;
                        fArr2[n][m] = (fArr2[n - 1][m] * cos) - (fArr2[n - 2][m] * k);
                        float[][] fArr3 = this.mPDeriv;
                        fArr3[n][m] = (((-sin) * fArr2[n - 1][m]) + (fArr3[n - 1][m] * cos)) - (fArr3[n - 2][m] * k);
                    }
                    m++;
                }
                n++;
    ...
    public float getFieldStrength() {
        ...
        f += f2 * f2;
        f2 = this.mZ;
        return (float) Math.sqrt((double) (f + (f2 * f2)));
    }
