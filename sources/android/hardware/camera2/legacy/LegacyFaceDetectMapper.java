...
public class LegacyFaceDetectMapper {
    ...
    public void processFaceDetectMode(CaptureRequest captureRequest, Parameters parameters) {
        ...
        Key key = CaptureRequest.STATISTICS_FACE_DETECT_MODE;
        boolean z = false;
        Integer valueOf = Integer.valueOf(0);
        int fdMode = ((Integer) ParamsUtils.getOrDefault(captureRequest, key, valueOf)).intValue();
        if (fdMode == 0 || this.mFaceDetectSupported) {
            int sceneMode = ((Integer) ParamsUtils.getOrDefault(captureRequest, CaptureRequest.CONTROL_SCENE_MODE, valueOf)).intValue();
