        ...
        synchronized (this.mSurfacesLock) {
            ...
            while (it.hasNext()) {
                ...
                for (int j2 = 0; j2 < configuredOutputs.size(); j2++) {
                    int streamId = sparseArray.keyAt(j2);
                    int surfaceId = 0;
                    for (Surface outSurface : ((OutputConfiguration) sparseArray.valueAt(j2)).getSurfaces()) {
                        if (s == outSurface) {
                            streamFound = true;
                ...
                if (!streamFound) {
                    ...
                    while (j < configuredOutputs.size()) {
                        int streamId2 = sparseArray.keyAt(j);
                        int surfaceId2 = 0;
                        for (Surface outSurface2 : ((OutputConfiguration) sparseArray.valueAt(j)).getSurfaces()) {
